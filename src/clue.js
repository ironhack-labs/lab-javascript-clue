...
const weaponsArray = [{
    name: "rope", weight: 10
}, { name: "knife", weight: 8 }, { name: "candlestick", weight: 2 }, { name: "dumbbell", weight: 30 }, { name: "poison", weight: 2 }, { name: "axe", weight: 15 }, { name: "bat", weight: 13 }, { name: "trophy", weight: 25 }, { name: "pistol", weight: 20 }
];
...
function selectRandom(array) {
    ...
    let randomCard = Math.floor(Math.random() * (array.length) + 0)
...
function pickMystery() {
        ...
        let cardSuspects = selectRandom(suspectsArray);
        let cardRooms = selectRandom(roomsArray);
        let cardWeapons = selectRandom(weaponsArray);
        const mistery = { suspect: cardSuspects, weapon: cardWeapons, room: cardRooms }
        return mistery
...
function revealMystery(mail) {

    const killer = (`${mail.suspect.firstName} ${mail.suspect.lastName} killed Mr. Boddy using the ${mail.weapon.name} in the ${mail.room.name}!`)
    return killer
