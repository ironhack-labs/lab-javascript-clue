...
function selectRandom(arr) {
    ...
    let emptyArray = arr.length === 0;
    let oneElementArray = arr.length === 1

    if(emptyArray) return undefined;
    else if(oneElementArray) return arr[0];
    else return arr[randomCard];
};

function pickMystery() {

    //should returns an object with three properties: suspect, weapon and room
    let mystery = {};
...
function revealMystery(mystery) {
    return `${mystery.suspect.name} ${mystery.suspect.lastname} killed Mr. Boddy using the ${mystery.weapon.name} in the ${mystery.room.name}!`
