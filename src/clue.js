// ITERATION 1
//Followed a workshop by Ironhack on 07/10 and did this game on codepen. Now adapting to Jasmine standards.
//Original game link https://codepen.io/ihatepineapple/pen/qBNBPea

// Suspects Collection
// Followed form workshop, created objects outside of array, then nested objects (by name only) inside of array
let mrGreen = {
    name: "Jacob Green",
...
// ITERATION 2
//Function select Random (followed the Math.random method)
...
function pickMystery(object){
    let suspect = selectRandom(suspectsArray)
    let room = selectRandom(roomsArray)
    let weapon = selectRandom(weaponsArray)

    return envelope = {suspect, weapon, room }
...
function revealMystery(envelope)  {
    ...
    return (`${envelope.suspect.name} killed Mr. Boddy using the ${envelope.weapon.name} in the ${envelope.room.name} !` )
}
