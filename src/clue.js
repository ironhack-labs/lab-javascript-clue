...
function selectRandom(guilty) {
    
    return (guilty[(Math.floor(Math.random() * guilty.length))]);
...
let criminoso = selectRandom(suspectsArray);
mistery.suspect = criminoso

let armaDoCrime = selectRandom(weaponsArray);
mistery.weapon = armaDoCrime

let localDoCrime = selectRandom(roomsArray);
mistery.room = localDoCrime
...
function revealMystery(envelope) {
    ...
    let weaponCrime = envelope.weapon.name

    let roomCrime = envelope.room.name

    let resolution = `${suspectFirstName} ${suspectLastName} killed Mr. Boddy using the ${weaponCrime} in the ${roomCrime}!`
