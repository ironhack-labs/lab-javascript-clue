...
function selectRandom (randomArray) {
    ...
    //console.log(Math.round(Math.random() * randomArray.length));
    //const arrLength = randomArray.length-1;
    return randomArray[Math.round(Math.random() * (randomArray.length-1))];
}

// Declare a function named pickMystery that takes no arguments and returns an object with three properties: 
// suspect, weapon and room, each holding as a value a card of that specific type. You can get a random card of 
// each type by calling selectRandom on each card stack.
