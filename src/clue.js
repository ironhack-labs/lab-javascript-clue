...
function selectRandom(arr) {
  counter = Math.floor(Math.random() * arr.length);
  return(arr[counter]);
}
...
function pickMystery() {
  ...
  suspectCard = selectRandom(suspectsArray);
  weaponCard = selectRandom(weaponsArray);
  roomCard = selectRandom(roomsArray);
...
function revealMystery(mysteryCardSet) {
  return (`${mysteryCardSet.suspect.firstName} ${mysteryCardSet.suspect.lastName} killed Mr. Boddy using the ${mysteryCardSet.weapon.name} in the ${mysteryCardSet.room.name}!`); 
}
