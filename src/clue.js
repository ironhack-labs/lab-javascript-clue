// ITERATION 1
/*Create an array for every card type. These arrays should be named suspectsArray, weaponsArray, roomsArray. 
...
function selectRandom(deck) {
  const numberOfElements = deck.length;
  if (numberOfElements > 1) {
    const randomPick = Math.floor(Math.random() * (numberOfElements - 1) + 1);
    return deck[randomPick];
  } else return deck[0];
}
