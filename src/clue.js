...
function selectRandom(arr){
    let selectedCard
    const anyKeys = Object.keys(arr)
    for (let i = 0; i < arr.length; i++) {
          arr[anyKeys[i]]
        }
      selectedCard = arr[Math.floor(Math.random()*arr.length)];
      return selectedCard;
  }
  ...
  function pickMystery(){
    ...
    mysteryObject.suspect = selectedSuspect
    mysteryObject.room = selectedRoom
    mysteryObject.weapon = selectedWeapon
