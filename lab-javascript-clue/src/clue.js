...
function selectRandom(arr){
  if ( arr=== roomsArray){
  let randomRoom = arr[Math.floor(Math.random()*arr.length)];
  return randomRoom.name;}
  else if ( arr === weaponsArray){
    let randomWeapond = arr[Math.floor(Math.random()*arr.length)];
    return randomWeapond.name;
  }
  else if (arr===suspectsArray){
    let randomPerson = arr[Math.floor(Math.random()*arr.length)];
...
function revealMystery() {
  let finale = pickMystery();
  return `${finale.suspect} killed Mr. Boddy using the ${finale.weapon} in the ${finale.room}!`;
}
