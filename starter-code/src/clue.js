...
function randomSelector(characters, rooms, weapons ) {
  var randomNumber = [];
  var random = [];
  var character = Math.floor((Math.random() * characters.length) + 1);
  var room = Math.floor((Math.random() * rooms.length) + 1);
  var weapon = Math.floor((Math.random() * weapons.length) + 1);
  randomNumber.push(character, room, weapon);
  random.push(characters[randomNumber[0]], rooms[randomNumber[1]], weapons[randomNumber[2]])
  return random[0]['first_name'] + ' ' + random[0]['last_name'] + ' killed Mr.Boddy using the ' + random[2].name + ' in the ' + random[1].name + '!!!!';
}
