...
let newArray = [...weaponsArray, ...roomsArray, ...suspectsArray];

let selectRandom = newArray => {
  let randomCard = newArray[Math.floor(Math.random() * newArray.length)];
  return randomCard;
};

selectRandom(newArray);

// takes no arguments and returns an object with three properties, suspect, weapon and room, each holding as a value a card of that specific type
