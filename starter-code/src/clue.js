...
function randomSelector(ArrayforSelection){
  let randomIndex = 0;
  if (ArrayforSelection.length==0){
    return undefined;
  } else if (ArrayforSelection.length==1){
    return ArrayforSelection[0];
  } else {
    randomIndex = parseInt(Math.random()*ArrayforSelection.length);
    if (randomIndex>0){
      return ArrayforSelection[randomIndex];
