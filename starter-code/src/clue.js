...
function randomSelector(randomArray){
    if (randomArray !== 0){
        if(randomArray.length === 1){
            ...
            let position = Math.floor(Math.random()*randomArray.length);
            return randomArray[position];
        }
    }else{
        return;
    }
...
function pickMistery (){
    let auxArray=[];
        auxArray.push(randomSelector(charactersArray));
        auxArray.push(randomSelector(weaponsArray));
        auxArray.push(randomSelector(roomsArray));
    return auxArray;
}
