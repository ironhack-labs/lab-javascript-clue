...
function randomSelector(arreglo) {
  if (arreglo.length < 0) {
    ...
    var max = arreglo.length - 1;
    var min = 0;
    var randomNumber = Math.floor(Math.random() * (max - min) + min);
   // console.log(randomNumber);
    return arreglo[randomNumber];
  }
}
...
function revealMistery(misteryEnvelope) {
  ...
  return misteryEnvelope.charactersArray.frst_name +" "+ misteryEnvelope.charactersArray.last_name + "killed Mr.Boddy using the "+ misteryEnvelope.weapon + "in the"+ misteryEnvelope.room;
}
