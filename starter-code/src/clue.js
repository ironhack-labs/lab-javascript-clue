...
{frst_name:"Doctor", last_name:"Orchid", color:"white", description:"PhD in plant toxicology. Adopted daughter of Mr. Boddy", age:26, image:"http://www.radiotimes.com/uploads/images/Original/111967.jpg", ocupation: "Scientist"},
...
function getTheMystery(numberOfSuspect, numberOfRooms, numberOfWeapons){
    var murderer=charactersArray[Math.floor(Math.random()*(numberOfSuspect))].first_name;
    var placeMurder=roomsArray[Math.floor(Math.random()*(numberOfRooms))];
    var weaponMurder=weaponsArray[Math.floor(Math.random()*(numberOfWeapons))].name;


     return murderer+" killed Mr Bobby in the "+ placeMurder +" with the "+ weaponMurder + ".";

};
console.log(getTheMystery(charactersArray.length, roomsArray.length, weaponsArray.length));
