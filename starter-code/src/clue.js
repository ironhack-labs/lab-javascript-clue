...
function randomSelector(Arr) {
  var selectedNumber = Math.floor(Math.random()*Arr.length);  
  return Arr[selectedNumber];
}

function pickMistery() {
  var Arr = [];
  Arr[0] = randomSelector(charactersArray);
  Arr[1] = randomSelector(weaponsArray);
  Arr[2] = randomSelector(roomsArray);
  return Arr;
}
...
function revealMistery(misteryEnvelope){
  var comment = ( misteryEnvelope[0].first_name + " " + misteryEnvelope[0].last_name + " killed Mr.Boddy using the " + misteryEnvelope[1].name + " in the " + misteryEnvelope[2].name + "!!!!");
  return comment;
}
