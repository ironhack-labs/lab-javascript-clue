...
describe('Reveal the mistery - revealMistery', function () {
  ...
  it('Return an array', function () {
    expect(typeof revealMistery([{ firstName: 'aa', lastName: 'abc' }, { name: 'abd' }, { name: 'abb' }])).toEqual('string');
  });

  it('Return <FIRST NAME> <LAST NAME> killed Mr.Boddy using the <WEAPON> in the <PLACE>!!!!', function () {
    expect(revealMistery([{ firstName: 'Victor', lastName: 'Plum' }, { name: 'poison' }, { name: 'Billiard Room' }])).toEqual('Victor Plum killed Mr.Boddy using the poison in the Billiard Room!!!!');
  });
});
